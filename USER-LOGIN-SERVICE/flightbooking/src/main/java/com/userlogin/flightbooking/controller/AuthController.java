   ...
   public String loginForm(@RequestParam String email, @RequestParam String password, HttpServletRequest request) {
       try {
           ...
           if ("ADMIN".equals(authenticatedUser.getRole())) {
               ...
               // Generate JWT and redirect to flight booking service
               String token = jwtService.generateToken(authenticatedUser, "USER");
               return "redirect:http://localhost:8082/search-service/flights/search?token=" + token;
           }
       } catch (RuntimeException e) {
